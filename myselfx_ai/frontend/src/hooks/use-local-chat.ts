...
function demoReply(userContent: string): string {
  const preview = userContent.trim().slice(0, 120);
  return [
    "**MyselfX AI · demo response**",
    "",
    `You asked: *"${preview}"*`,
    "",
    "This live demo is currently running in **offline demo mode** because the AI backend is not connected. When the FastAPI backend is deployed and `NEXT_PUBLIC_WS_URL` is configured, this chat streams live responses from a real PydanticAI agent (with tool calls and conversation history).",
    "",
    "What you can explore right now:",
    "- **New Chat** - start and rename conversation threads",
    "- **History** - conversations are persisted in your browser",
    "- **Theme** - toggle light/dark mode from the header",
    "",
    "Get in touch: myselfxdeveloper@gmail.com",
  ].join("\n");
}
...
export function useLocalChat() {
  ...
  const sendChatMessage = useCallback(
    (content: string) => {
      ...
      // When the backend WebSocket is unreachable (e.g. standalone Vercel demo),
      // fall back to a simulated local response so the demo never breaks.
      if (!isConnected) {
