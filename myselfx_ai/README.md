# MyselfX AI — Full-Stack AI Assistant

<p align="center">
  <b>Real-time AI chat assistant with streaming responses, secure JWT auth, and conversation persistence.</b><br>
  <sub>Built on FastAPI + MySQL-adjacent SQLite, Next.js 15, and PydanticAI.</sub>
</p>

<p align="center">
  <b>Live:</b> <a href="https://myselfx-ai.vercel.app">myselfx-ai.vercel.app</a> &nbsp;•&nbsp;
  <b>Author:</b> Moeen Ul Qadir &nbsp;•&nbsp;
  <b>Email:</b> <a href="mailto:myselfxdeveloper@gmail.com">myselfxdeveloper@gmail.com</a> &nbsp;•&nbsp;
  <b>Phone:</b> <a href="tel:+923441586424">+923441586424</a>
</p>
...
MyselfX AI is a production-grade full-stack AI assistant. It pairs a **FastAPI**
backend (JWT auth, SQLite/ORM, and a PydanticAI streaming agent) with a
**Next.js 15** frontend that features real-time WebSocket chat, tool-call
visualization, and browser-persisted conversations.

Built, branded, and maintained by **Moeen Ul Qadir** — full stack software
engineer based in Multan, Punjab, Pakistan.
...
Each generated project includes a CLI tool named `myselfx_ai`. Run commands from the `backend/` directory:

```bash
cd backend

# Server commands
uv run myselfx_ai server run --reload     # Start dev server
uv run myselfx_ai server routes           # Show all routes
# Database commands
uv run myselfx_ai db migrate -m "message" # Create migration
uv run myselfx_ai db upgrade              # Apply migrations
uv run myselfx_ai db downgrade            # Rollback migration
# User commands
uv run myselfx_ai user create-admin       # Create admin user
uv run myselfx_ai user create             # Create regular user
uv run myselfx_ai user list               # List all users
```
...
# Server
uv run myselfx_ai server run --reload
uv run myselfx_ai server routes

# Database (Alembic wrapper)
uv run myselfx_ai db init
uv run myselfx_ai db migrate -m "Add users"
uv run myselfx_ai db upgrade

# Users
uv run myselfx_ai user create-admin       # Create admin (interactive)
uv run myselfx_ai user create             # Create user (interactive)
uv run myselfx_ai user list               # List all users
```
...
```bash
uv run myselfx_ai cmd seed --count 100
uv run myselfx_ai cmd seed --dry-run
```
...
| Start dev server | `uv run --directory backend myselfx_ai server run --reload` |
| Start prod server | `uv run --directory backend myselfx_ai server run --host 0.0.0.0 --port 8000` |
| Show routes | `uv run --directory backend myselfx_ai server routes` |
...
| Task | Command |
|------|---------|
| Create migration | `uv run --directory backend myselfx_ai db migrate -m "message"` |
| Apply migrations | `uv run --directory backend myselfx_ai db upgrade` |
| Rollback migration | `uv run --directory backend myselfx_ai db downgrade` |
| Show current | `uv run --directory backend myselfx_ai db current` |
| Show history | `uv run --directory backend myselfx_ai db history` |

### Users

| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend myselfx_ai user create-admin` |
| Create user | `uv run --directory backend myselfx_ai user create` |
| List users | `uv run --directory backend myselfx_ai user list` |
...
The frontend is ready to deploy to Vercel as-is.

1. Push this repository to GitHub.
2. Import the repo into Vercel.
3. Set **Root Directory** to `myselfx_ai/frontend` (a repo-root `vercel.json` is included to do this automatically).
4. Add optional env vars: `BACKEND_URL`, `NEXT_PUBLIC_WS_URL`, `NEXT_PUBLIC_SITE_URL`.
5. Deploy — recommended URL: **https://myselfx-ai.vercel.app**

> The frontend includes an offline demo mode, so the deployed site works even
> before the FastAPI backend is live. To enable real AI streaming, deploy the
> backend (e.g. on Render/Railway/VPS) and point `NEXT_PUBLIC_WS_URL` to it.
...
<p align="center">
  Made with ❤️ by <a href="https://www.moeenqadir.com">Moeen Ul Qadir</a>
</p>
